"""
OpenAI Provider for Tascade AI.
...
class OpenAIProvider(BaseAIProvider):
    """OpenAI provider for AI services in Tascade AI."""
    ...
    def __init__(self, api_key: Optional[str] = None, model: str = "gpt-4o"):
        ...
        # Import openai here to avoid dependency issues if not using this provider
        try:
            import openai
    ...
    def _call_openai(self, prompt: str, system_prompt: Optional[str] = None, temperature: float = 0.7) -> str:
        """
    ...
    def analyze_task(self, task: Task) -> Dict[str, Any]:
        """
        Analyze a task using OpenAI and provide insights.
        ...
        response = self._call_openai(prompt, system_prompt, temperature=0.2)
    ...
    def decompose_task(self, task: Task, project_rules: List[ProjectRule], 
        ...
        """
        Decompose a task into subtasks using OpenAI.
        ...
        response = self._call_openai(prompt, system_prompt, temperature=0.3)
    ...
    def generate_implementation_guide(self, task: Task, project_rules: List[ProjectRule]) -> str:
        """
        Generate an implementation guide for a task using OpenAI.
        ...
        return self._call_openai(prompt, system_prompt, temperature=0.4)
    
    def generate_verification_criteria(self, task: Task) -> str:
        """
        Generate verification criteria for a task using OpenAI.
        ...
        return self._call_openai(prompt, system_prompt, temperature=0.3)
    
    def apply_rules_to_task(self, task: Task, rules: List[ProjectRule]) -> List[Dict[str, Any]]:
        """
        Apply project rules to a task and get recommendations using OpenAI.
    ...
    def analyze_task_complexity(self, task: Task) -> Dict[str, Any]:
        """
        Analyze the complexity of a task using OpenAI.
    ...
    def generate_dependency_graph(self, task: Task, all_tasks: Dict[str, Task], format: str = "text") -> str:
        """
        Generate a dependency graph for a task using OpenAI.
        ...
        if format == "json":
            ...
            # For text format, we'll use OpenAI to generate a readable representation
            prompt = f"""
            ...
            return self._call_openai(prompt, system_prompt, temperature=0.1)
